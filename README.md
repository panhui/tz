...
列表显示每台服务器的「今日上传」「今日下载」。顶部使用三张卡片：实时上传/下载速度、今日上传/下载总流量、昨日上传/下载总流量，不再显示累计总流量卡片。三张卡片均随分组切换：选择「全部服务器」时汇总全部节点，选择分组时只统计该分组。速度只统计在线节点，今日和昨日流量包含该分组的离线节点，同一节点只计一次。按北京时间（UTC+8）每天 00:00 将今日数据结转到昨日，面板重启后保留。
